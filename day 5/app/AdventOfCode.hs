...
module AdventOfCode (ingestFile) where

import Data.List.Split (splitOn)
import Data.Monoid (Any)

type DestenationStart = Integer

type SourceStart = Integer
...
type ItemMap = [Integer]

isInRange :: Integer -> Integer -> Integer -> Integer
isInRange n i r
  | n == i = 0
  | n > i && n <= maxRange = abs (i - n)
  | otherwise = -1
  where
    maxRange = i + r

changeSourceToDest :: Integer -> ItemMap -> Integer
changeSourceToDest n [dest, source, range]
  | (-1) == mapping = n
  | otherwise = dest + mapping
  where
    mapping = isInRange n source range

sourceDestTuple :: ItemMap -> Integer -> ([Integer], [Integer])
sourceDestTuple b a
  | a == result = ([a], [])
  | a /= result = ([], [result])
  where
    result = changeSourceToDest a b

mapSourceDest :: [Integer] -> ItemMap -> [([Integer], [Integer])]
mapSourceDest a b = map (sourceDestTuple b) a

mapResultToGroups :: [Integer] -> [Integer] -> [([Integer], [Integer])] -> ([Integer], [Integer])
mapResultToGroups groupA groupB testData = (testA testData, testB testData)
-- mapResultToGroups groupA groupB ((a, b) : xs)
--   | remaining > 0 = mapResultToGroups (groupA ++ a) (groupB ++ b) xs
--   | otherwise = (groupA ++ a, groupB ++ b)
--   where
--     remaining = length xs
-- testA::  [([Integer], [Integer])] -> [Integer]
unwrapA :: ([Integer], [Integer]) -> [Integer]
unwrapA (a, _)= a
unwrapB :: ([Integer], [Integer]) -> [Integer]
unwrapB (_, b)= b
testA :: [([Integer], [Integer])] -> [Integer]
testA =  concatMap unwrapA
testB ::  [([Integer], [Integer])] -> [Integer]
testB = concatMap unwrapB


tryMap :: [Integer] -> ItemMap -> ([Integer], [Integer])
tryMap a b = mapResultToGroups [] [] $ mapSourceDest a b

-- mapAtoB :: [Integer] -> [Integer] -> [ItemMap] -> [Integer]
-- mapAtoB output a (b : bs)
--   | itemsRemaining == 0 = output ++ remaining ++ mapped
--   | otherwise = mapAtoB (output ++ mapped) remaining bs
--   where
--     (remaining, mapped) = tryMap a b
--     itemsRemaining = length bs
...
filterOutEmptyStrings :: [String] -> [String]
...
listOfNumberStringsToIntList :: [String] -> [Integer]
listOfNumberStringsToIntList = map toInt

listOfIntsToTriple :: [c] -> [c]
listOfIntsToTriple (a : b : c : is) = [a, b, c]

mapIntsToTriples :: [[c]] -> [[c]]
mapIntsToTriples = map listOfIntsToTriple

getSeeds :: String -> [Integer]
getSeeds str = do
  let (_ : seeds : xs) = splitOnColon str
  listOfNumberStringsToIntList (splitOnSpace seeds)

-- getMaps output (x : xs) = do
--     let isNewMap = length (splitOnColon x)

-- isEndOfMap = "" ==

isEmptyList :: [a] -> Bool
isEmptyList ls = not (null ls)

filterEmptyLists = filter isEmptyList
...
--     where
--         remaining = length xs
--         endOfMap = isEndOfMap x

getMapFromString :: String -> [Integer]
getMapFromString str = listOfNumberStringsToIntList (splitOnSpace str)

mapMapToInts :: String -> [ItemMap]
mapMapToInts str = do
  let (title : body : xs) = splitOnColon str
  let x = splitOnSpace body
  let y = map splitOnSpace $ filterOutEmptyStrings $ splitOnNewLine body
  mapIntsToTriples (map listOfNumberStringsToIntList y)

mapValuesAtoB :: [Integer] -> [[ItemMap]] -> [Integer]
mapValuesAtoB seeds (x : xs)
  | remaining == 0 = result
  | otherwise = mapValuesAtoB result xs
  where
    remaining = length xs
    result =  mapAtoB [] seeds x

-- testOther :: [Integer] -> [[ItemMap]] -> t2
-- testOther seeds = foldr (mapAtoB [] seeds) []

calcValues seeds (x : xs) = mapAtoB [] seeds x
...
ingestFile str = do
  ...
  let seedStr = head fileLines
  let fileBody = drop 1 fileLines
  -- print fileBody
  let seeds = getSeeds (head fileLines)
  let body = drop 1 (splitOnEmptyLine str)
  -- print body
  let test = map mapMapToInts body
--   print (map (map length) (inlineData))
  -- print test
  -- print seeds
  print (mapValuesAtoB seeds test)

-- minimum (mapValuesAtoB seeds test)
